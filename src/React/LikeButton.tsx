...
const LikeButton = () => {
  ...
  const handleLike = () => {
    setIsLiked((prevIsLiked) => !prevIsLiked); // Alternar el estado
    setTriggerAnimation(true); // Activar la animación
    setTimeout(() => {
      setTriggerAnimation(false); // Desactivar la animación después de 300 ms
    }, 300);
  };
  ...
  return (
    <div className="flex items-center">
      ...
      >
        ...
        <span
          className={`
          text-sm pl-3 transition-all duration-300 ease-in-out
          text-[var(--white)]
        `}
        >
          {isLiked ? "Te encanta" : "Me encanta"}
        </span>
      </button>
    </div>
  );
